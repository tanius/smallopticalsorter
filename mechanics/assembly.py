def m(id, part = None):
    ...
    @details Keeping all measures in one file is the simplest way to ensure we'll not have 
    ...
    if   id == "general: panel t":        return 3     # 3 mm polycarbonate walls are enough for this small machine.
    elif id == "general: fdm wall t":     return 3     # Default wall thickness for 3D printing (6 shells of 0.4 mm).
    elif id == "general: bolt t":         return 4     # M4 as standard bolt size.
    elif id == "general: tube r":         return 32    # Outer diameter of EN 1451 DN32 tubes.
    elif id == "general: tube wall t":    return 3     # Using EN 1451 DN32 tubes.
    ...
    elif id == "case: h":                 return m("case: cuboid h") + 20  # 20 mm max. space for protruding elements.
    elif id == "case: cuboid w":          return m("case: w") - 15  # @todo: Calculate as "max w - protruding parts".
    elif id == "case: cuboid d":          return m("case: d") - 20  #@todo: Calculate as "max d - protruding parts".
    # Wall height is calculated so that side and top walls are of the same size. (Top walls overlap side walls.)
    ...
    # @todo: Outsource these calculations into generic_case.scad, where they are contained redundantly right now.
    ...
    elif id == "upper belt: w":           return m("general: panel t") + 1 + m("upper belt: belt w") + 1 + m("general: panel t")
    ...
    elif id == "funnel: upper w":         return m("general: fdm wall t") + 2 + m("upper belt: w") + 2 + m("general: fdm wall t")
    ...
    elif id == "funnel: output w offset": return 0  # x axis offset of the output relative to being centered below the input.
    elif id == "funnel: output d offset": return 30 # y axis offset of the output relative to a vertical wall going down from the input.
