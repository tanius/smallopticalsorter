...
class Chute:
    ...
    def __init__(self, workplane, measures):
        ...
        :param workplane: The workplane to create the chute on.
        :param measures: The measures to use for the parameters of this design. Available values and defaults:
            - **``h``**: Height of the chute in total, in its rotated position.
            - **``d``**: Depth of the chute in total, in its rotated position.
            - **``wall_thickness``**: Wall thickness of the chute's side walls, measured vertically to the wall.
            - **``upper_w``**: Outer width of the chute at its input.
            - **``lower_w``**: Outer width of the chute at its output.
            - **``lower_straight_wall_h``**: Height of the straight part of the chute's lower U profile's side walls, measured vertically 
            ...
            - **``lower_rounded_wall_h``**: Height of the rounded part of the chute's lower U profile's side walls, measured vertically 
            ...
            - **``left_studs``**: Positions of stud mounts on the chute's left side face, given as a list of tuples. Each tuple 
            ...
            - **``left_wall_distance``**: Gap between the chute and the left wall to which to mount it, at the narrowest point.
            - **``right_studs``**: Positions of stud mounts on the chute's left side face. See left_studs for the format.
            - **``right_wall_distance``**: Gap between the chute and the right wall to which to mount it, at the narrowest point.
        ...
        .. todo:: *(Later)* Add chamfers where the studs connect to the chute. (Better than fillets, as they are 3D printable.)
        .. todo:: *(Later)* Make the stud supports for FDM 3D printing optional.
        .. todo:: *(Later)* Use an elliptical arc instead of a circular arc. That allows deep chutes and also avoids the problem of 
            arcs being more than a half circle sometimes. 
        ...
        .. todo:: *(Later)* If necessary, cut off the upper chute end somewhat (but not vertically down, that would be too much).
        .. todo:: *(Later)* If necessary, add that a width (x axis) offset of the output can be configured.
        .. todo:: *(Later)* Refactor the uProfile() straight_h and rounded_h parameters to be more natural and less technical. As in: both 
        ...
        .. todo:: *(Later)* Fix that, depending on the wall_thickness setting and esp. for larger ones, CadQuery might choose a different 
            mode of lofting, which connects the two profiles in ways that do not lead to a smooth rounded edge along the chute.
        .. todo::  *(Later)* Support different widths at the front and back of the profile. That allows to visually correct that cutting 
        ...
        .. todo:: *(Later)* Check for the error condition mentioned in the class docstring, and correct it automatically, with a hint 
            to the user.
        """
    ...
    def build(self):
        ...
        slide_angle = degrees(asin(self.h / slide_length)) # Drop angle at entry to the chute, same as exit angle.
        ...
        right_case_plane = cq.Workplane("YZ").workplane(offset = -self.right_wall_distance, invert = True)
...
def chute(self, measures):
    """
    Companion CadQuery plugin to make class Chute usable like any other CadQuery Workplane method.
    :param measures: A dict with the parameters for this design. See class Chute.
    """
    
    # In CadQuery plugins, it is good practice to not modify self, but to return a new object linke to self as a parent:
    # https://cadquery.readthedocs.io/en/latest/extending.html#preserving-the-chain .
    return self.newObject(
        Chute(self, measures).model.objects
    )
...
chute = cq.Workplane("XY").chute(measures)
